...
import { getSummarizerByUserId, getSummarizerHistory, deleteSummarizerById, storeAiGeneratedSummarize  } from '../services/summarizer';
import { getSummarizeOpenAiRes } from '../open-ai';
export const getSummarizerHistoryContorller = async (req: Request, res: Response) => {
...
export const deleteSummarizerByIdController = async (req: Request, res: Response) => {
    const { userId, id } = req.params;
    try {
        const userSummarizer = await deleteSummarizerById(id, userId);
        if (!userSummarizer) {
            return res.status(404).json({
                message: 'Document not found',
                status: 404,

...
import {
  getAiDetctionHistory,
  getAiDetectionById,
  getPlagDetectionById,
  getPlagrismDetectionHistory,
} from "../services/contentDetection";
...
export const getContentDetectionResponse = async (
  req: Request,
  res: Response
) => {
  const { userId, method, content, stripe_subscription_id } = req.body;
  ...
  try {
    ...
    if (method === "Plagiarism Detection") {
      const responseFromCopyscape = (await generateCopyscapeResponse(
        content
      )) as IPlagrismDetection;
      ...
      const response = new PlagrismDetection({
        method,
        userId,
        data: {
          prompt: content,
      ...
      const newResponse = new AiDetection({
        method,
        userId,
        data: {
          prompt: content,
...
export const getContentDetectionHistory = async (
  req: Request,
  res: Response
) => {
...
export const getContentDetectionHistoryById = async (
  req: Request,
  res: Response
) => {

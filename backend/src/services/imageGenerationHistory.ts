...
export const storeGeneratedImageInHistory = async (
  userId: string,
  prompt: string,
  image_b64: String,
...
export const getUserImageHistory = async (userId: string) => {
  try {
    ...
    const imageGeneration = await ImageGeneration.find({ userId })
      .select("_id")
      .select("createdAt")
      .select("prompt")
      .select("revised_prompt");
    if (!imageGeneration) {
      return {
...
export const getGeneratedImageByUserId = async (
  imageId: string,
  userId: string
) => {
  try {
...
export const deleteGeneratedImageByUserId = async (
  id: string,
  userId: string
) => {
  try {
    ...
    const imageGeneration = await ImageGeneration.findByIdAndDelete({
      _id: id,
      userId,
    });
    if (!imageGeneration) {
...
export const getDownloadableImage = async (
  imageId: string,
  resolution: string
) => {
  try {

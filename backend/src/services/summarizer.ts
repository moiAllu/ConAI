...
export const getSummarizerByUserId = async (
  summarizeId: string,
  userId: string
) => {
  try {
    const userSummarizer = await Summarize.findOne(
      { userId },
      { summarizes: { $elemMatch: { _id: summarizeId } } }
    );
    if (!userSummarizer) {
...
export const deleteSummarizerById = async (id: string, userId: string) => {
  try {
    const userSummarizer = await Summarize.findOne({ userId });
    if (!userSummarizer) {
      return new Error("Document not found");
    }
    const summarizeIndex = userSummarizer.summarizes.findIndex(
      (summarize) => summarize._id.toString() === id.toString()
    );
    if (summarizeIndex === -1) {
      return new Error("Document not found");
    }
    userSummarizer.summarizes.splice(summarizeIndex, 1);
    await userSummarizer.save();
    return userSummarizer;
  } catch (e) {
    console.log(e);
    return null;
...
export const storeAiGeneratedSummarize = async (
  intensity: string,
  content: string,
  userId: string,
  output: string
  ...
  try {
    ...
    const newSummarize = new Summarize({
      userId: userId,
      rewrites: [{ input: content, output, created_at: new Date(), intensity }],
    });

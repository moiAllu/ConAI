...
export function categorizeChatMessages(chatMessages: ChatMessage[]): IndexedChatMessages[] {
  const categorizedMessages: { [key: string]: ChatMessage[] } = {};
  ...
  const indexedCategories: IndexedChatMessages[] = Object.keys(categorizedMessages).map(category => ({
      category,

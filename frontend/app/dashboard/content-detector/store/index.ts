...
interface Actions {
  addAiHistory: (
    aiHistory: IAiDetection,
    userId: string,
    method: string,
  ) => void;
  addPlagrismHistory: (
    plagrismHistory: IPlagrismDetection,
    userId: string,
    method: string,
  ) => void;
  addCompareHistory: (
...
export const useContentDetectorStore = create<States & Actions>()(
  persist(
    (set) => ({
      aiHistory: {
        method: "",
      ...
      addAiHistory: (aiHistory, userId, method) => {
        set((state) => {
          ...
          const existing = _.find(newState.aiHistory.aiDetectionHistory, {
            _id: aiHistory._id,
          });
          if (existing) {
            return newState;
            ...
            newState.aiHistory = {
              ...
              userId,
              aiDetectionHistory: [
                ...newState.aiHistory.aiDetectionHistory,
                aiHistory,
              ],
            };
            return newState;
      ...
      addPlagrismHistory: (plagrismHistory, userId, method) => {
        set((state) => {
          ...
          const existing = _.find(
            newState.plagrismHistory.plagrismDetectionHistory,
            { _id: plagrismHistory._id },
          );
          if (existing) {
            return newState;
          } else {
      ...
      addCompareHistory: (compareHistory, userId, method) => {
        set((state) => {
          ...
          newState.compareHistory = {
            ...
            userId,
            compareHistory: [
              ...newState.compareHistory.compareHistory,
              compareHistory,
            ],
          };
          return newState;
